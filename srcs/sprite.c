...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/07/06 09:09:26 by mskinner          #+#    #+#             */
...
void	sort_sprite(t_info *info, t_player *player)
{
	...
	{
		info->sprite[i].distance = hypot(player->x - info->sprite[i].x,
...
{
	s_ray->height = (int)fabs(info->height / s_ray->transform_y);
...
{
	s_ray->width = (int)fabs(info->height / s_ray->transform_y);
...
void	get_sprite_color(t_info *info, t_sprite_ray *s_ray, int stripe)
{
	...
	s_ray->tex_x = (int)((256 * (stripe - (-s_ray->width / 2 + s_ray->screen_x))
